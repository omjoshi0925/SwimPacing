...
the project repository, and the pseudonymous race table is deposited at
Harvard Dataverse \citep{joshi2026dataset}. The rows as published in the
source results --- name, club, age and time together --- are not
redistributed, because that combination re-identifies minors; the chain from
official result sheet to every number in this paper can be re-executed by
anyone holding those public sheets, whose digests are recorded (see Data and
code availability).
